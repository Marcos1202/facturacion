...
/* * ************************************************** */
const TIPODOCREFVALUES = array('01','02','03','04','05','06','07','08','09','11','12','99');
...
/*}*/
    ...
    foreach ($detalles as $d)
    {
        ...
        if (isset($d->codigoComercial) && $d->codigoComercial != ""){
            //Delimita el array a solo 15 elementos
    ...
    if (isset($otros) && $otrosType != ''){
        ...
        if (in_array($otrosType, $tipos))
        {
            ...
            foreach($otros as $otr){
               ...
               $xmlString .= '<' . $otrosType . ' codigo="'.$otr->codigo.'" >' . $otr->texto . '</' . $otrosType . '>';
            }
...
function genXMLMr()
{
    ...
    $CondicionImpuesto              = params_get("condicion_impuesto");                       // d18,5 opcional /obligatorio si comprobante tenga impuesto
    $MontoTotalImpuestoAcreditar    = params_get("monto_total_impuesto_acreditar");                       // d18,5 opcional /obligatorio si comprobante tenga impuesto
    $codigoActividad                = params_get("codigo_actividad");                            // https://cloud-cube.s3.amazonaws.com/sp5z9nxkd1ra/public/assets/json/actividades_por_codigo.json
    ...
    if (!empty($montoTotalImpuesto)){
        ...
        $montoTotalImpuesto . '</MontoTotalImpuesto>';
    }
    if (!empty($CondicionImpuesto)){

        $xmlString .= '<CondicionImpuesto>' . 
        $CondicionImpuesto . '</CondicionImpuesto>';
    }
    if (!empty($MontoTotalImpuestoAcreditar)){

        $xmlString .= '<MontoTotalImpuestoAcreditar>' . 
        $MontoTotalImpuestoAcreditar . '</MontoTotalImpuestoAcreditar>';
    }
